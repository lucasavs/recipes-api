...
class RecipeDto {
    ...
    async getAllRecipe(): Promise<Recipe[]> {
        const [rows, fields] = await pool.query<RowDataPacket[]>('SELECT * FROM recipes');
    ...
    async updateRecipe(recipe: Recipe): Promise<Recipe> {
        ...
        try {
            const [rows, fields] = await pool.query<RowDataPacket[]>('SELECT * FROM recipes WHERE id= ?', [recipe.id]);
            if (rows.length === 0) {
            ...
            const [updatedRows, updatedFields] = await pool.query('SELECT * FROM recipes WHERE id= ?', [recipe.id]);
